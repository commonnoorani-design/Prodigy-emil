#!/usr/bin/env node
/**
 * MCP server for Prodigy Educations Mail.
 *
 * Speaks to a running instance over its HTTP API using a personal access
 * token, so an assistant can read the inbox and send branded mail as you.
 * It holds no mail credentials of its own: everything goes through the same
 * routes and the same permission checks as the web app.
 *
